...
if __name__ == "__main__":
    ...
    # parser.add_argument('--abi_path', type=str, default=abi_file_path)
    parser.add_argument('--daily_gas', type=float, default=0.1)
    # parser.add_argument('--whitelist_address', type=str, default=whitelist_address)
    ...
    print(f'daily gas {args.daily_gas} ETH')
    user_input = input('continue(y)? ')
    if user_input!='y':
        sys.exit()

    this_function = proxy_contract.functions.setDailyGasLimit(int(args.daily_gas*1e18))
